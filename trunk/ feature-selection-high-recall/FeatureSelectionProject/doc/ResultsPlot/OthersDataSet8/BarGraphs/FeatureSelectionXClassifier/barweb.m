...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	% Plot bars
	handles.bars = bar(barvalues, width,'edgecolor','k');
	hold on
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		handles.legend = legend(bw_legend, 'location', 'SouthOutside');
		legend boxoff;
	else
		handles.legend = [];
    end
	...
	for i = 1:numbars
		...
		x = mean(x([1 3],:));
		handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), Upper(:,i), 'k', 'linestyle', 'none');
	...
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'xtick',1:numgroups,'xgrid','off','ygrid','off');
