...
for c = 1:nCl
    ...
    %loop over metrics
    nM = size(data.Simulation.DataSet{1}.Classifier{c}.Metric,2);
    for m = 1:nM
      MName = data.Simulation.DataSet{1}.Classifier{c}.Metric{m}.Attributes.name;
      
      %compute average and plot
      Y = zeros(nDS,nAlg);
      Error = zeros(nDS,nAlg);
      dataSetLegends = {};
      for d=1:nDS  
          ...
          for alg = 1:nAlg
            algname = data.Simulation.DataSet{d}.Classifier{c}.Metric{m}.Algorithms.Algorithm{alg}.Attributes.name;
            %xpath expression
            ...
            Y(d,alg) = acc/count;
            Error(d,alg) = 2 * (std(samples)/sqrt(length(samples)));
          end;
          %normalize
          Y(d,:) = Y(d,:) ./ max(Y(d,:));
      end;
      %plot graph
       %createBarGraph(Y,dataSetLegends,legendKeys, cleanString(strjoin({CName, MName},' ')), 'Average', 'Data Sets')
       figure, barweb(Y, Error, 1, dataSetLegends, cleanString(strjoin({CName, MName},' ')), 'Data Sets', 'Average', [], [],legendKeys , 2, 'plot')
      %save as pdf
      saveas(gcf, strjoin({CName, MName},' ') , 'pdf')
    end;
end;
