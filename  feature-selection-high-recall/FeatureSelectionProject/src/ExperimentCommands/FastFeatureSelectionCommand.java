...
public class FastFeatureSelectionCommand implements IExperimentCommand{
	...
	public FastFeatureSelectionCommand(List<ELinearClassifier> classifiers, List<EFeatureSelectionAlgorithm> FetSelectionAlgs, String graphPath, int kMaxFeatures){
		...
		for (ELinearClassifier ecl :  this.classifiers){
			...
			for (EFeatureSelectionAlgorithm alg : this.selectionAlgs) {
				...
				try {
					...
					int[][] fold2PreviousSelectedFeature = new int[10][];
					...
					for (int n_features = 1; n_features <= maxNumFeatures; n_features++) {
						...
						CrossValidationOutput outlr = eval.fastCrossValidation(cl, randData, n_features, alg, 10, param, fold2PreviousSelectedFeature);
